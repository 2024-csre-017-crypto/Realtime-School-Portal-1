...
import { useGetStudents, useCreateStudent, useDeleteStudent } from "@workspace/api-client-react";
import { useQueryClient } from "@tanstack/react-query";
import { Card, Button, Input, Modal, Select } from "@/components/ui-elements";
...
export default function AdminStudents() {
  ...
  const [formData, setFormData] = useState({
    name: "", password: "", class: "", father: "", phone: "", dob: "", address: "", rollNo: ""
  });

  const queryClient = useQueryClient();
  const { mutate: create, isPending: isCreating } = useCreateStudent();
  const { mutate: remove, isPending: isDeleting } = useDeleteStudent();
  ...
  const handleCreate = (e: React.FormEvent) => {
    ...
    create({ data: formData }, {
      onSuccess: () => {
        ...
        setFormData({ name: "", password: "", class: "", father: "", phone: "", dob: "", address: "", rollNo: "" });
      }
    });
  };
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr><td colSpan={5} className="text-center py-8 text-muted-foreground">No students found</td></tr>
              ) : (
        ...
        <form onSubmit={handleCreate} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input value={formData.class} onChange={e => setFormData({...formData, class: e.target.value})} required placeholder="e.g. 10A" />
            </div>
            <div>
          ...
          <div className="pt-4 flex gap-3">
            ...
            <Button type="submit" className="flex-1" isLoading={isCreating}>Create Student</Button>
          </div>
        </form>
      </Modal>
    </div>
