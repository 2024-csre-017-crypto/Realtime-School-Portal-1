  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`/api/diary/${studentId}`, {
        ...
        credentials: "include",
        body: JSON.stringify({ date, subject, note, imageUrl }),
      });
      ...
      <Card className="p-6 md:p-8">
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-muted-foreground mb-2">
                Attach Image <span className="text-xs opacity-60">(optional — saved to cloud)</span>
              </label>
              <input ref={fileInputRef} type="file" accept="image/*" onChange={handleImageChange} className="hidden" id="diary-image" />
              {!imagePreview ? (
                <label htmlFor="diary-image" className="flex items-center gap-3 px-4 py-3 rounded-xl border border-dashed border-white/20 text-muted-foreground hover:border-primary hover:text-primary cursor-pointer transition-colors">
          ...
          <Button type="submit" className="w-full" isLoading={busy}>
            <Send className="w-4 h-4" /> {isUploading ? "Uploading image…" : "Send Entry"}
          </Button>
        </form>
      </Card>
    </div>
